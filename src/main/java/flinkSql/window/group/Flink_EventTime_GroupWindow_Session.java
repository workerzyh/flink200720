 ...
 */
//TODO 事件时间滑动窗口
public class Flink_EventTime_GroupWindow_Session {
    public static void main(String[] args) throws Exception {
       ...
       //转换表添加事件时间
        ...
        //table API风格
        Table tableResult = table.window(Session.withGap("10.seconds").on("rt").as("rw"))
                .groupBy("rw,id")
                .select("id,id.count");

        //SQL风格
        tableEnv.createTemporaryView("sqlTable",table);
        Table sqlResult = tableEnv.sqlQuery("select id,count(id) from sqlTable group by id,session(rt,interval '10' second)");
